...
describe('Text', () => {
  ...
  it('should fail if group name is undefined', () => {
    expect(() => CustomText.create(undefined as any)).toThrowError();
  });

  it('should fail if group name is null', () => {
    expect(() => CustomText.create(null as any)).toThrowError();
  });

  it('should fail if group name has 0 characters', () => {
    expect(() => CustomText.create('')).toThrowError();
  });

  it('should fail if group name has 9 characters', () => {
    expect(() => CustomText.create('a'.repeat(9))).toThrowError();
  });

  it('should create a group name with 10 characters', () => {
    expect(CustomText.create('a'.repeat(10))).toBeTruthy();
  });

  it('should create a group name with 100 characters', () => {
    expect(CustomText.create('a'.repeat(100))).toBeTruthy();
  });

  it('should fail if group name has 101 characters', () => {
    expect(() => CustomText.create('a'.repeat(101))).toThrowError();
  });
});
